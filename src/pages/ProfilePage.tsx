...
const ProfilePage = () => {
  return (
    <div className="max-w-5xl mx-auto space-y-8 pb-16">
      ...
      {/* Page Header */}
      <div>
        <h1 className="text-2xl font-bold text-slate-900 px-1">Business Profile</h1>
        <p className="text-slate-500 font-medium px-1">360° view of your business</p>
      </div>

      {/* Business Identity Card */}
      <div className="relative group overflow-hidden rounded-3xl border border-slate-200 bg-white p-2 shadow-sm transition-all hover:shadow-lg hover:border-orange-100">
        ...
        <div className="relative p-6 sm:p-8 flex flex-col lg:flex-row lg:items-center justify-between gap-8">
            <div className="flex items-start sm:items-center gap-6">
                <div className="h-20 w-20 sm:h-24 sm:w-24 shrink-0 rounded-[2rem] bg-orange-50 flex items-center justify-center p-5 shadow-inner border border-orange-100/50">
                    <Building2 className="h-full w-full text-[#F07B20]" />
                </div>
                <div>
                    <h2 className="text-2xl sm:text-2xl font-bold text-slate-900 leading-tight">Mama Fua Laundry & Dry Cleaning</h2>
                    <p className="text-slate-400 font-bold uppercase tracking-wider text-[10px] mt-1">Retail</p>
                    ...
                    <div className="mt-6 flex items-center gap-3">
                        <div className="flex items-center justify-center h-8 w-8 rounded-full bg-orange-100 text-[#F07B20] shadow-sm">
                            <Edit3 size={14} />
                        </div>
                        <span className="text-xs font-bold text-slate-500 uppercase tracking-widest">Profile Builder</span>
                    </div>
                </div>
            </div>

            <div className="flex flex-col items-center lg:items-end gap-3 shrink-0">
                <div className="bg-[#F07B20] text-white px-10 py-5 rounded-[2.5rem] shadow-lg shadow-orange-100 transition-transform hover:scale-102 cursor-default">
                ...
                <button className="flex items-center gap-2 px-6 py-2.5 rounded-2xl bg-white border border-slate-200 text-slate-600 font-bold text-sm shadow-sm hover:bg-slate-50 transition-all">
                    <Edit3 size={16} /> Edit
                </button>
            </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden flex flex-col">
            <div className="p-8 border-b border-slate-100 flex items-center justify-between">
                <h3 className="text-lg font-bold text-slate-900 tracking-tight">Key Metrics</h3>
            ...
            <div className="p-8 flex-1 flex flex-col justify-around gap-6">
                {[
                    ...
                    <div key={i} className="flex items-center justify-between group">
                        <span className="text-slate-500 font-bold text-sm group-hover:text-slate-900 transition-colors uppercase tracking-wider">{metric.label}</span>
                        <div className="flex items-center gap-2">
                            {metric.icon && <metric.icon size={16} className={metric.color} />}
                            <span className={`text-xl font-bold tracking-tight ${metric.color}`}>{metric.value}</span>
                        </div>
                    </div>
                ))}
            </div>
        ...
        <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
            ...
            <div className="p-8">
                <div className="relative h-[220px] w-full">
                    ...
                    {/* Y-Axis Labels */}
                    <div className="absolute left-0 top-0 h-full flex flex-col justify-between text-[10px] font-bold text-slate-400 -translate-x-full pr-4">
                        <span>120K</span>
                        <span>90K</span>
                        <span>60K</span>
                        <span>30K</span>
                    ...
                    {/* X-Axis Labels */}
                    <div className="absolute bottom-[-24px] w-full flex justify-between text-[10px] font-bold text-slate-400">
                        <span>Jul</span>
                        <span>Aug</span>
                        <span>Sep</span>
                        <span>Oct</span>
                ...
                <div className="mt-12 flex items-center gap-2 text-[10px] font-bold text-slate-400 uppercase tracking-widest">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
            ...
            { label: 'Satisfaction', value: '4.7/5.0', color: 'text-slate-900' },
            { label: 'Corporate', value: '35%', color: 'text-slate-900' }
        ].map((item, i) => (
            <div key={i} className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm transition-transform hover:-translate-y-1">
                <h4 className="text-[10px] font-bold text-slate-400 uppercase tracking-widest leading-none mb-3">{item.label}</h4>
      ...
      <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
        ...
        <div className="p-8 grid grid-cols-1 md:grid-cols-3 gap-8">
            <div className="flex items-center gap-4">
                <div className="h-12 w-12 rounded-2xl bg-slate-50 flex items-center justify-center text-slate-400 group-hover:bg-white border border-slate-100">
                    <Phone size={20} />
                </div>
                <div>
            ...
            <div className="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p className="text-sm font-bold text-slate-900">mamafua@example.com</p>
                </div>
            </div>
            <div className="flex items-center gap-4">
                <div className="h-12 w-12 rounded-2xl bg-slate-50 flex items-center justify-center text-slate-400 group-hover:bg-white border border-slate-100">
                ...
                <div>
                    ...
                    <p className="text-sm font-bold text-slate-900">Ngara, Nairobi</p>
                </div>
            </div>
        </div>
      </div>
      ...
      {/* Final CTA Action */}
      <div className="pt-4">
          <button className="w-full bg-[#0f172a] text-white py-6 rounded-[2rem] font-bold text-lg uppercase tracking-widest shadow-2xl shadow-slate-200 hover:bg-slate-800 transition-all flex items-center justify-center gap-4 group">
              <FileText className="h-6 w-6 text-[#F07B20]" />
